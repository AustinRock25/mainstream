...
export const authenticate = (req, res, next) => {
  ...
  verify(token, process.env.JWT_SECRET, (error, decoded) => {
    if (error)
      ...
      query("SELECT u.id AS id, email, password, is_admin, rs.id AS rating_scale, rs.min AS min, rs.max AS max FROM users u LEFT JOIN rating_scales rs ON rs.id = u.rating_scale WHERE u.id = $1", [decoded.id])
        .then(results => {
