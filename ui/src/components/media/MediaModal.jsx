        ...
        <Modal.Title className="w-100">
          <div className="d-flex align-items-center justify-content-between">
            ...
            <div className="d-flex flex-column flex-md-row gap-xs-1 gap-md-4">
              ...
              {media.type == "movie" && (!user ? <span className={`fw-bold fs-5 text-${Math.round((Number.parseFloat(media.grade) + Number.parseFloat(100)) / 2) <= 69 ? "danger" : Math.round((Number.parseFloat(media.grade) + Number.parseFloat(100)) / 2) <= 79 ? "warning" : "success"}`}>{getGrade(media)}</span> : <span className={`fw-bold fs-5 text-${Math.round(((Math.round((Number.parseFloat((media.grade / 100) * (user.rating_scale_max - user.rating_scale_min)) + Number.parseFloat(user.rating_scale_min)) * (1 / user.rating_scale_step)) / (1 / user.rating_scale_step)) - user.rating_scale_min) / (user.rating_scale_max - user.rating_scale_min) * 100) <= 39 ? "danger" : Math.round(((Math.round((Number.parseFloat((media.grade / 100) * (user.rating_scale_max - user.rating_scale_min)) + Number.parseFloat(user.rating_scale_min)) * (1 / user.rating_scale_step)) / (1 / user.rating_scale_step)) - user.rating_scale_min) / (user.rating_scale_max - user.rating_scale_min) * 100) <= 60 ? "warning" : "success"}`}>{getGrade(media)}</span>)}
              {media.type == "show" && (!user ? <span className={`fw-bold fs-5 text-${Math.round((Number.parseFloat(media.grade_tv) + Number.parseFloat(100)) / 2) <= 69 ? "danger" : Math.round((Number.parseFloat(media.grade_tv) + Number.parseFloat(100)) / 2) <= 79 ? "warning" : "success"}`}>{getGrade(media)}</span> : <span className={`fw-bold fs-5 text-${Math.round(((Math.round((Number.parseFloat((media.grade_tv / 100) * (user.rating_scale_max - user.rating_scale_min)) + Number.parseFloat(user.rating_scale_min)) * (1 / user.rating_scale_step)) / (1 / user.rating_scale_step)) - user.rating_scale_min) / (user.rating_scale_max - user.rating_scale_min) * 100) <= 39 ? "danger" : Math.round(((Math.round((Number.parseFloat((media.grade_tv / 100) * (user.rating_scale_max - user.rating_scale_min)) + Number.parseFloat(user.rating_scale_min)) * (1 / user.rating_scale_step)) / (1 / user.rating_scale_step)) - user.rating_scale_min) / (user.rating_scale_max - user.rating_scale_min) * 100) <= 60 ? "warning" : "success"} mb-0`}>{getGrade(media)}</span>)}
            </div>
          </div>
        </Modal.Title>
      </Modal.Header>
        ...
        <Row>
          <Col xs={12} md={4} className="text-center mb-4 mb-md-0">
            ...
            <Stack direction="horizontal" gap={3} className="justify-content-center align-items-center mb-4 mb-md-0">
              ...
              {media.type == "show" && media.seasons.length > 1 && (!user ? <p className={`fw-bold fs-5 text-${Math.round((Number.parseFloat(media.seasons[currentSeason].grade) + Number.parseFloat(100)) / 2) <= 69 ? "danger" : Math.round((Number.parseFloat(media.seasons[currentSeason].grade) + Number.parseFloat(100)) / 2) <= 79 ? "warning" : "success"} mb-0`}>{getGradeSeason(media.seasons[currentSeason])}</p> : <p className={`fw-bold fs-5 text-${Math.round(((Math.round((Number.parseFloat((media.seasons[currentSeason].grade / 100) * (user.rating_scale_max - user.rating_scale_min)) + Number.parseFloat(user.rating_scale_min)) * (1 / user.rating_scale_step)) / (1 / user.rating_scale_step)) - user.rating_scale_min) / (user.rating_scale_max - user.rating_scale_min) * 100) <= 39 ? "danger" : Math.round(((Math.round((Number.parseFloat((media.seasons[currentSeason].grade / 100) * (user.rating_scale_max - user.rating_scale_min)) + Number.parseFloat(user.rating_scale_min)) * (1 / user.rating_scale_step)) / (1 / user.rating_scale_step)) - user.rating_scale_min) / (user.rating_scale_max - user.rating_scale_min) * 100) <= 60 ? "warning" : "success"} mb-0`}>{getGradeSeason(media.seasons[currentSeason])}</p>)}
            </Stack>
          </Col>
          <Col xs={12} md={8}>
            <div className="mb-4">
                      ...
                      <Accordion.Body className="bg-dark text-white-50">
                        <Row>
                          ...
                          <Col xs={12} md={4} className="text-center mb-1 mb-md-0">
                            <img 
                              src={`posters/${new Date(media.seasons[0].episodes[0].release_date).getUTCFullYear()}_${getPoster(media)}/${currentSeason + 1}x${ep.episode}.jpg`}
                              alt={media.title}
