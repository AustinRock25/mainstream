        ...
        <Modal.Title className="w-100">
          <div className="d-flex align-items-center justify-content-between">
            ...
            <div className="d-flex flex-column flex-md-row gap-xs-1 gap-md-4">
              ...
              {media.type == "movie" && (<span className={`fw-bold fs-5 text-${Math.round(media.grade) <= 39 ? "danger" : Math.round(media.grade) <= 60 ? "warning" : "success"}`}>{getGrade(media)}</span>)}
              {media.type == "show" && (<span className={`fw-bold fs-5 text-${Math.round(media.grade_tv) <= 39 ? "danger" : Math.round(media.grade_tv) <= 60 ? "warning" : "success"} mb-0`}>{getGrade(media)}</span>)}
            </div>
          </div>
        </Modal.Title>
      </Modal.Header>
        ...
        <Row>
          <Col xs={12} md={4} className="text-center mb-4 mb-md-0">
            ...
            <Stack direction="horizontal" gap={3} className="justify-content-center align-items-center mb-4 mb-md-0">
              ...
              {media.type == "show" && media.seasons.length > 1 && (<p className={`fw-bold fs-5 text-${Math.round(media.seasons[currentSeason].grade) <= 39 ? "danger" : Math.round(media.seasons[currentSeason].grade) <= 60 ? "warning" : "success"} mb-0`}>{getGradeSeason(media.seasons[currentSeason])}</p>)}
            </Stack>
          </Col>
          <Col xs={12} md={8}>
            <div className="mb-4">
