  ...
  return (
    <Modal show={show} onHide={handleClose} size="lg" centered contentClassName="bg-dark text-white">
      <Modal.Header closeButton closeVariant="white">
        <Modal.Title><i>{media.title}</i> <span className="fw-light fs-5 text-white-50 d-inline-flex me-3">{getYear(media)} <Stack direction="horizontal" gap={3} className="justify-content-center"> <span>{media.rating === "Not Rated" ? "NR" : media.rating}</span> <span>{time(media.runtime)}</span> <span className="fw-bold">{getGrade(media.grade || media.grade_tv)}</span></Stack></span></Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {media.type == "show" &&
          <Row>
      ...
      {(!!user && media.type == "movie") && <MediaForm show={showMediaForm} setShow={setShowMediaForm} media={media} />}
      {(!!user && media.type == "show") && <MediaForm show={showMediaForm} setShow={setShowMediaForm} media={media.seasons[currentSeason]} />}
    </Modal>
  );
}
