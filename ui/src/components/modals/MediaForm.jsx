                  ...
                  {episodes.map((episode, index) => (
                    <div key={index} className="border p-3 mb-3 bg-dark rounded">
                      <div className="d-flex mb-2">
                        ...
                        <Form.Control className="me-2" type="date" value={episode.release_date || ""} onChange={(e) => handleEpisode(e, "release_date", index)} />
                        <Form.Control type="number" value={episode.runtime || ""} placeholder="Runtime (mins)" onChange={(e) => handleEpisode(e, "runtime", index)} />
                        <Button variant="outline-danger" className="ms-2" onClick={() => handleRemoveEpisode(index)}>X</Button>
                      </div>
                      <Row className="mt-3">
                        <Form.Control as="textarea" rows={12} value={episode.synopsis || ""} onChange={(e) => handleEpisode(e, "synopsis", index)} />
                      </Row>
                      <Row className="mt-3">
                        <Col md={6}>
                          <h6>Available</h6>
                  ...
                  <Form.Group as={Row} className="mb-3">
                    ...
                    <Col sm={9}>
                      <Form.Select value={formData.rating} onChange={(e) => handleChange(e, "rating")}>
                        {formData.type !== "movie" ? Array.isArray(["Not Rated", "TV-Y", "TV-Y7", "TV-Y7 FV", "TV-G", "TV-PG", "TV-14", "TV-MA"]) && ["Not Rated", "TV-Y", "TV-Y7", "TV-Y7 FV", "TV-G", "TV-PG", "TV-14", "TV-MA"].map(r => <option key={r} value={r}>{r}</option>) : Array.isArray(["Not Rated", "G", "PG", "PG-13", "R", "NC-17"]) && ["Not Rated", "G", "PG", "PG-13", "R", "NC-17"].map(r => <option key={r} value={r}>{r}</option>)}
                      </Form.Select>
                    </Col>
                  </Form.Group>
                )}
